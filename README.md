...
Это монорепозиторий **продуктовой документации и реализации** Belcanto Product (PD-0031). Мобильный клиент, серверный API и их контракты развиваются рядом с нормативным корпусом, но не заменяют его.

Здесь хранятся нормативные документы Belcanto и регистрируются внешние источники, которые определяют продукт: зачем он существует, для кого, из чего состоит, как устроен, как выглядит и в каком порядке создаётся.
...
│   └── api/                     ← модульный Go API
├── contracts/
│   └── http/                    ← версия HTTP-контракта Belcanto API
├── .github/workflows/           ← проверки корпуса, mobile и API
│
├── docs/
...
|----------------|-----------|
| `Project · docs/project/` | `Approved` — Конституция проекта 1.1.3 |
...
| `product/` | `000`–`003`, `006`, `011` — `Approved`; `005`, `009` — `Draft`; `004`, `007`, `008`, `010` — `Scaffold` |
| `language/` | `000`, `001` — `Draft` в M-0003; `002`–`005` — `Scaffold` |
| `domain/` | документы в каскаде M-0003 — `Draft`; остальные сохраняют свои legacy-статусы до отдельной миграции |
| `experience/` | `Scaffold` |
| `features/` | `Scaffold` |
| `design/` | `Scaffold` |
| `roadmap/` | `Scaffold` |
| `governance/` | `decisions/002` — `Approved`; остальные документы — `Scaffold` |
| `ai/` | `Scaffold` |
| `assets/` | `Scaffold` |

Реализация вертикального среза B.0 ведётся в изолированной рабочей ветке: Owner или Administrator с делегированным разрешением создаёт Student; назначенный Teacher публикует First Belcanto Minute; только Owner выпускает, перевыпускает или отзывает Invitation; Student активирует Account и входит. Наличие кода не повышает статус производных продуктовых документов и не заменяет их человеческую рецензию P7.
