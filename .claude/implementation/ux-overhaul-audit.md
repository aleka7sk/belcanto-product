...
| ux3 (231ee5a) | TK-03 (закрыт: 7 последних legacy-экранов + app.config.ts), TK-05 (закрыт: legacy-слой удалён из tokens.ts, остались 10 B.0-декоративных размеров под re-skin), LST-01 (staff: занятия future/journal и ростер/ревью на ScreenList), LST-02/03 (teacher-зона: retry+PTR на Today/Lesson/AssessmentDetail/Moderation/Series/Operations), CON-03 (частично: LessonCard) | Отложено с причинами: Moderation/Repertoire/EventsCatalog остаются map (ограниченные объёмы); 11 account-экранов не рендерили ошибку загрузки и до overhaul (существующий пробел, не регресс) |

## Реестр непроверенного (обязательный)

Требует устройства/человека — headless-сессия это проверить не может:
1. **Визуальная сверка 242 Figma-состояний** пофреймно (Pages 19–37) — скриншоты на устройстве против фреймов; структурные контракты (нав-хост, раскадровка, геометрия целей) сверены по метаданным, пиксели — нет.
2. **Dynamic Type на железе**: рост контейнеров при ×1.5–×3 (minHeight-полы поставлены, но реальный рендер шрифтов надо смотреть глазами).
3. **VoiceOver/TalkBack-проход**: порядок фокуса, объявление tablist/tab/switch/radio, layout после hitSlop.
4. **60fps на длинных лентах** (активность/сообщество/занятия) — профилирование на устройстве.
5. **Pull-to-refresh жест** на всех экранах с RefreshControl — жестовая механика поверх FlatList/ScrollView.
6. **kk-строки** (com.safety.*, prac.submit.open/review.open) — перевод реализации, ждёт носителя языка.
7. **Поведение replace-навигации на Android** (аппаратная «назад» после переключения табов).

## Чеклист владельца (что проверить на устройстве)

Контур ученика (+77001234569 / belcanto-student-2026!):
- Дом: таб-бар прибит к низу, не скроллится; «Повторить» при выключенном API — рабочая кнопка.
- Расписание: нав прибит; свайп вниз обновляет; чипы дней ≥48pt по ширине.
- Урок: нав прибит; таб «Сообщество» доступен.
- Практика: список заданий, свайп-обновление; деталь задания — «Сдать работу» открывает отдельный экран сдачи.
- Прогресс: обзор без форм; «Цель» и «Достижения» — отдельные экраны по ссылкам.
- Сообщество: лента виртуализирована; «Пожаловаться» под комментарием — легко попасть пальцем (48pt); жалоба и блокировка — отдельные экраны, тред чистый.
- Активность: секции Сегодня/Ранее, свайп-обновление, «Повторить» работает.
- Переключение табов не наращивает стек (кнопка «назад» не разматывает историю табов).

Контур педагога (+77001234568 / belcanto-teacher-2026!):
- Таб «Сегодня» → кокпит дня (не общий хаб); таб «Расписание» → внутренние занятия (не гард «Раздел недоступен»).
- «Ученики»/«Ревью» — сегменты переключаются и подсвечивают свой таб; повторный тап по табу «Ревью» переключает сегмент.
- Занятия: секции «Будущие»/«Журналы» в виртуальном списке, PTR.

Контур администратора/владельца (+77001234567 / belcanto-local-password-2026):
- Админ: таб «Люди» → рабочее пространство С таб-баром; «Ещё» подсвечивается на экранах аккаунта.
- Владелец: таб «Команда» → делегирование С таб-баром; «Аналитика» подсвечивает свой таб и открывает свой сегмент.
- Все создающие формы (занятие/ученик/смена педагога) — с таб-баром, без тупиков.
